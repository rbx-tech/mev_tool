...
class BundleTasks:
    ...
    def create(self, start_time, end_time):
        ...
        if task:
            ...
            query = f"""
                INSERT INTO bundle_tasks (start_time, end_time)
    ...
    def get_by_time_range(self, start_time, end_time):
        query = f"""
            SELECT id FROM bundle_tasks WHERE start_time = {start_time} AND end_time = {end_time}
        """
    ...
    def update_done(self, task_id, count: int):
        query = f"""
            UPDATE bundle_tasks SET is_done = TRUE, count = {count} WHERE id = {task_id}
        """
        self.db.execute(query)